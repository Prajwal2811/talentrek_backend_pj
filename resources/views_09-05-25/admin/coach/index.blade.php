                            ...
                            <div class="body">
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover dataTable js-exportable">
                                        ...
                                        <tbody>
                                            @foreach($coaches->unique('id') as $index => $coach)
                                            <tr>
                                                ...
                                                <td>
                                                    <label class="switch"
                                                        @if($coach->status === 'inactive' && !empty($coach->inactive_reason))
                                                            data-bs-toggle="tooltip"
                                                            data-bs-placement="top"
                                                            title="Reason: {{ $coach->inactive_reason }}"
                                                        @endif>
                                                        <input type="checkbox"
                                                            {{ $coach->status === 'active' ? 'checked' : '' }}
                                                            onchange="toggleStatus(this)"
                                                            data-mentor-id="{{ $coach->id }}">
                                                ...
                                                <script>
                                                    ...
                                                    function toggleStatus(checkbox) {
                                                        const coahcId = $(checkbox).data('mentor-id');
                                                        const isChecked = checkbox.checked;

                                                        if (!isChecked) {
                                                            currentCheckbox = checkbox;
                                                            $('#modal-mentor-id').val(coahcId);
                                                            $('#inactive-reason-input').val('');
                                                            modalInstance.show();
                                                        } else {
                                                            sendStatusUpdate(coahcId, 'active');
                                                        }
                                                    }
                                                    ...
                                                    function submitInactiveReason() {
                                                        const coahcId = $('#modal-mentor-id').val();
                                                        ...
                                                        modalInstance.hide();
                                                        sendStatusUpdate(coahcId, 'inactive', reason);
                                                    }


                                                    function sendStatusUpdate(coahcId, status, reason = null) {
                                                        $.ajax({
                                                            url: '{{ route('admin.coach.changeStatus') }}',
                                                            method: 'POST', 
                                                            data: {
                                                                _token: '{{ csrf_token() }}',
                                                                coach_id: coahcId,
                                                ...
                                                <td>{{ \Carbon\Carbon::parse($coach->created_at)->format('d/m/Y') }}</td>
                                                <td>
                                                    <a href="{{ route('admin.coach.view', $coach->id) }}" class="btn btn-sm btn-primary">View Profile</a>
                                                    <a href="{{ route('admin.coach.booking-session', $coach->id) }}"  class="btn btn-sm btn-success text-light">Session History</a>
                                                </td>
                                            </tr>
                                            @endforeach
                                        </tbody>

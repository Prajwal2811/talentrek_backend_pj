            ...
            <main class="p-6 bg-gray-100 flex-1 overflow-y-auto" x-data="dashboard()">
                ...
                <div x-data="sessionManager()" class="bg-white p-6 rounded-lg shadow relative">
                    ...
                    <div class="flex space-x-6 border-b mb-4 text-sm font-medium text-gray-600">
                        ...
                        <button @click="switchTab('cancelled')" :class="tabClass('cancelled')">{{ langLabel('cancelled_sessions') }}
                            </button>
                        <button @click="switchTab('completed')" :class="tabClass('completed')">{{ langLabel('completed_sessions') }}
                            </button>
                    </div>
                    <!-- Session List -->
                    <template x-for="(session, index) in paginatedSessions()" :key="index">
                        <div class="flex justify-between items-center border-b py-4">
                            ...
                            <div class="flex flex-1 justify-evenly  ml-10 text-sm text-gray-700">
                                <div>
                                    <p class="font-semibold">{{ langLabel('session_date') }}</p>
                                    <p x-text="session.date"></p>
                                </div>
                                <div>
                                    <p class="font-semibold">{{ langLabel('session_time') }}</p>
                    ...
                    <div x-show="showCancelModal" x-transition 
                        ...
                        <form id="basic-form" method="POST" novalidate action="{{ route('coach.dashboard-action') }}">
                            @csrf
                            ...
                            <div class="bg-white rounded-lg shadow-lg w-full max-w-3xl p-6 relative">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end mb-6">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ langLabel('postpose_session') }}</label>

<?php
    ...
    ->selectSub(function ($query) use ($mentorId) {
        $query->from('messages as m')
            ->selectRaw('COUNT(*)')
            ->where('m.sender_type', 'jobseeker')
            ->where('m.receiver_id', $mentorId)
            ->where('m.receiver_type', 'mentor')
            ->where('m.is_read', 0)
                                        ...
                                        <template x-for="(msg, index) in activeContact.messages" :key="index">
                                            <div class="flex items-end gap-2" :class="msg.sender === 'me' ? 'justify-end' : 'justify-start'">
                                                ...
                                                <div :class="msg.sender === 'me'
                                                    ...
                                                    x-html="msg.html"></div>
                                                <img x-show="msg.sender === 'me'" :src="mentorImage" class="w-8 h-8 rounded-full object-cover">
                                            </div>
                                        </template>
                                    </div>
                        ...
                        window.mentorChat = function () {
                            return {
                                ...
                                sendMessage() {
                                    ...
                                    fetch('/mentor/chat/send', { method: 'POST', body: formData })
                                        .then(res => res.json())
                                        .then(data => {

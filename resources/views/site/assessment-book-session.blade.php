    ...
    <div class="page-content">
        <div class="relative bg-center bg-cover h-[400px] flex items-center" style="background-image: url('{{ asset('asset/images/banner/Assessment.png') }}');">
            ...
            <div class="relative z-10 container mx-auto px-4">
                <div class="space-y-2">
                    <h2 class="text-5xl font-bold text-white ml-[10%]">{{ langLabel('assessment') }}</h2>
                </div>
            </div>
        </div>
    </div>
       ...
       @if(session('success'))
        ...
        @endif
          ...
          <main class="w-11/12 mx-auto py-8">
            @php
                // Mentor session fee
                $sessionFee = $assessorDetails->per_slot_price ?? ($assessor->per_slot_price ?? 0);

                // Tax model for mentors (always percentage)
                $taxation = App\Models\Setting::first();
                $taxRate = floatval($taxation->assessorTax ?? 0);
            ...
            <div class="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between p-6 rounded-lg bg-white">
                        <div class="flex items-center gap-6">
                            @php
                                ...
                                $avgRating = number_format($assessor->reviews->avg('ratings') ?: 0, 1);
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-700 mt-0.5">
                                    {{ $assessor->total_experience ?? '0 years' }} of experience
                                </p>
                    ...
                    <section class="max-w-7xl mx-auto p-4">
                        <form method="POST" action="{{ route('session.booking-submit') }}" x-data="{ paymentMethod: '' }" id="assessorBookingForm">
                            @csrf
                            ...
                            <div class="max-w-6xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                                <div class="lg:col-span-2 space-y-6">
                                    ...
                                    <div class="grid grid-cols-2 gap-6 mb-6">
                                        <div>
                                            <input type="hidden" name="user_type"  value="assessor" />
                                            <input type="hidden" name="user_id" id="mentorIdInput" value="{{ $assessorDetails->assessor_id }}" />
                                            <input type="hidden" name="jobseeker_id" value="{{ optional(auth('jobseeker')->user())->id }}" />

                                            <label class="block font-semibold mb-2">{{ langLabel('assessor_mode') }}</label>
                        ...
                        $(function () {
                            ...
                            $('#bookBtn').on('click', function(e) {
                                ...
                                if (!mode) {
                                    $('#modeSelect').after('<span class="text-red-500 error-message">Please select assessor mode</span>');
                                    isValid = false;
                                }
                                if (!date) {
                        ...
                        function fetchSlots() {
                            ...
                            const mentorId = document.getElementById('mentorIdInput').value;
                            ...
                            const url = `{{ route('get-assessor-available-slots') }}?mode=${encodeURIComponent(mode)}&date=${encodeURIComponent(date)}&assessor_id=${encodeURIComponent(mentorId)}`;

    ...
    <div class="page-wraper">
        <div class="flex h-screen">
            ...
            <main class="p-6 bg-gray-100 flex-1 overflow-y-auto" x-data="createBookingSlots()">
                ...
                <h2 class="text-2xl font-semibold mb-4">{{ langLabel('booking_slots') }}</h2>
                <div class="flex gap-6">
                    ...
                    <div class="bg-white p-4 rounded-lg shadow w-1/3">
                        ...
                        <p class="text-xs text-gray-500 mt-2">
                            {{ langLabel('note_mark_unavailable') }}
                        </p>
                    </div>
                            ...
                            <tbody>
                                <template x-for="(slot, index) in activeSlots" :key="index">
                                    <tr class="border-b">
                                        ...
                                        <td class="px-4 py-2">
                                            <div class="flex items-center gap-2" x-data="{ updateUrl: @js(route('mentor.update-slot-status')) }">
                                                <input 
                                                    type="checkbox" 
                                ...
                                >
                                    <div class="bg-white rounded-lg p-6 w-full max-w-md" @click.outside="showDeleteModal = false">
                                        ...
                                        <p class="text-sm text-gray-700 mb-6">{{ langLabel('delete_slot_confirm') }}</p>
...
function createBookingSlots() {
    ...
    return {
        ...
        markUnavailable() {
            ...
            const updateUrl = '{{ route('mentor.update-slot-status') }}';
        ...
        // updateSlotTime() {
        //     fetch('{{ route('mentor.update-slot-time') }}', {
        //         method: 'POST',
        //         headers: {
        //             'Content-Type': 'application/json',
        //             'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        confirmDelete() {
            fetch('{{ route('mentor.delete-slot') }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'

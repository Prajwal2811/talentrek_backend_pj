    ...
    <script>
        ...
        @endif
            ...
            <main class="w-11/12 mx-auto py-8">
                @php
                    // Mentor session fee
                    $sessionFee = $mentorDetails->per_slot_price ?? ($mentor->per_slot_price ?? 0);

                    // Tax model for mentors (always percentage)
                    $taxation = App\Models\Setting::first();
                    $taxRate = floatval($taxation->mentorTax ?? 0);
                ...
                <div class="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-col md:flex-row md:items-center justify-between p-6 rounded-lg bg-white">
                            <div class="flex items-center gap-6">
                                @php
                                    ...
                                    $avgRating = number_format($mentor->reviews->avg('ratings') ?: 0, 1);
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-700 mt-0.5">
                                        {{ $mentor->total_experience ?? '0 years' }} of experience
                                    </p>
                        ...
                        <section class="max-w-7xl mx-auto p-4">
                            <form method="POST" action="{{ route('mentorship-booking-submit') }}" x-data="{ paymentMethod: '' }" id="mentorshipBookingForm">
                                @csrf
                            ...
                            function fetchSlots() {
                                ...
                                const url = `{{ route('get-available-slots') }}?mode=${encodeURIComponent(mode)}&date=${encodeURIComponent(date)}&mentor_id=${encodeURIComponent(mentorId)}`;

<?php
    ...
    ->selectSub(function ($query) use ($assessorId) {
        $query->from('messages as m')
            ->selectRaw('COUNT(*)')
            ->where('m.sender_type', 'jobseeker')
            ->where('m.receiver_id', $assessorId)
            ->where('m.receiver_type', 'assessor')
            ->where('m.is_read', 0)
            ->whereColumn('m.sender_id', 'j.id'); // <-- correlated subquery
    }, 'unread_count')
    ->distinct()
    ->get();
                                        ...
                                        <template x-for="(msg, index) in activeContact.messages" :key="index">
                                            <div class="flex items-end gap-2" :class="msg.sender === 'me' ? 'justify-end' : 'justify-start'">
                                                ...
                                                <div :class="msg.sender === 'me'
                                                    ...
                                                    x-html="msg.html"></div>
                                                <img x-show="msg.sender === 'me'" :src="assessorImage" class="w-8 h-8 rounded-full object-cover">
                                            </div>
                                        </template>
                                    </div>
                        ...
                        window.assessorChat = function () {
                            return {
                                ...
                                sendMessage() {
                                    ...
                                    fetch('/assessor/chat/send', { method: 'POST', body: formData })
                                        .then(res => res.json())
                                        .then(data => {

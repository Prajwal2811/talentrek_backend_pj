@if(auth()->user('jobseeker'))
    @php
        $paymentHistory = App\Models\PaymentHistory::where('user_type', 'jobseeker')
            ->where('user_id', auth()->user('jobseeker')->id)
            ->get()
            ->map(function($payment) {
                $payer = App\Models\Jobseekers::find($payment->user_id);
                ...
                <table class="min-w-full text-sm text-left border rounded-lg overflow-hidden shadow-sm">
                    <thead class="bg-gray-100 text-gray-700">
                        <tr>
                            ...
                            <th class="px-3 py-2 border">Paid To</th>
                    ...
                    <tbody class="text-gray-700">
                        @foreach($paymentHistory as $index => $payment)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-3 py-2">{{ $payment->receiver_name }}</td>
                                <td class="px-3 py-2">{{ \Carbon\Carbon::parse($payment->paid_at)->format('d/m/Y H:i') }}</td>
                                <td class="px-3 py-2 font-medium">{{ $payment->currency }} {{ number_format($payment->amount_paid, 2) }}</td>
                                <td class="px-3 py-2">

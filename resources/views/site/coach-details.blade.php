            ...
            <div class="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between p-6 rounded-lg">
                    ...
                    <div class="mt-4 md:mt-0">
                        @if($coach->bookingSlots->isNotEmpty())
                            @php
                                $firstSlot = $coach->bookingSlots->first();
                            @endphp
                            @auth('jobseeker')
                            ...
                            @elseif(auth('expat')->check())
                                <a href="{{ route('expat.coach-book-session', ['coach_id' => $coach->id, 'slot_id' => $firstSlot->id]) }}">
                                    <button class="bg-blue-800 hover:bg-blue-900 text-white px-6 py-2 rounded-md font-medium shadow-sm transition">
                                        {{ langLabel('book_session') }}
                                    </button>
                                </a>
               ...
               @php
                    ...
                    $avgRating = $totalReviews > 0 ? round($coach->reviews->avg('ratings'), 1) : 0;

                    $ratingsPercent = [];
                    foreach ([5, 4, 3, 2, 1] as $star) {
                        $count = $coach->reviews->where('ratings', $star)->count();
                ...
                <section id="reviews">
                    ...
                    <script>
                        ...
                        document.getElementById('submit-review').addEventListener('click', function () {
                            ...
                            fetch("{{ route('submit.coach.review') }}", {
                                method: "POST",
                                headers: {

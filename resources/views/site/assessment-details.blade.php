            ...
            <div class="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between p-6 rounded-lg">
                    ...
                    <div class="mt-4 md:mt-0">
                        @if($assessor->bookingSlots->isNotEmpty())
                            @php
                ...
                <div class="flex gap-6 border-b mb-6 text-sm font-medium">
                    <a href="#about" class="tab-link pb-2 text-gray-600 hover:text-blue-600 border-b-2 border-transparent hover:border-blue-600">{{ langLabel('about_mentor') }}</a>
                ...
                <section id="about" class="mb-6 mt-6">
                    <h2 class="text-lg font-semibold mb-2">{{ langLabel('about_assessor') }}</h2>
                    <p class="text-sm text-gray-700 mb-6">
                        {!! nl2br(e($assessor->about_assessor ?? langLabel('bio_not_available'))) !!}
                    </p>
                    ...
                    @php
                        $education = $assessor->educations->first();
                    @endphp
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        ...
                        <div>
                            <p class="font-semibold text-gray-800">{{ langLabel('fieled_of_study') }}:</p>
               ...
               @php
                    $totalReviews = $assessor->reviews->count();
                    $avgRating = $totalReviews > 0 ? round($assessor->reviews->avg('ratings'), 1) : 0;

                    $ratingsPercent = [];
                    foreach ([5, 4, 3, 2, 1] as $star) {
                        $count = $assessor->reviews->where('ratings', $star)->count();
                ...
                <section id="reviews">
                    ...
                    <!-- Hidden Assessor ID -->
                    <input type="hidden" id="assessor_id" value="{{ $assessor->id }}">
                    ...
                    <script>
                        ...
                        document.getElementById('submit-review').addEventListener('click', function () {
                            ...
                            fetch("{{ route('submit.assessor.review') }}", {
                                method: "POST",
                                headers: {

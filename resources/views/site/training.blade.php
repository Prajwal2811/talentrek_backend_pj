        ...
        <main class="w-3/4 mx-auto" x-data="courseFilter()">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <span class="text-sm text-gray-500">
                    {{ langLabel('showing') }} <span x-text="filteredCourses.length"></span>
                    {{ langLabel('total_results') }}
                </span>
            </div>
            ...
            <div class="mb-6 relative">
                <input type="text" placeholder="{{ langLabel('search_here') }}..." x-model="search"
                    @input="filterCourses" class="w-full border border-gray-300 rounded-md px-4 py-2 pr-12" />
                    ...
                    <div class="course-item flex border rounded-md overflow-hidden shadow-sm mb-4 bg-white">
                        ...
                        <div style="min-width: 250px; max-width: 250px; overflow: hidden;">
                            <img :src="material.thumbnail_file_path 
                                            ...
                                            : '/default.png'" alt="Thumbnail" class="h-full w-full object-cover" />
                        </div>
                        ...
                        <div class="p-4 flex-1 flex flex-col justify-between">
                            <div>
                                ...
                                @elseif(auth('expat')->check())
                                    <a :href="`{{ route('expat.course.details', '') }}/${material.id}`">
                                        <h2 class="font-semibold text-lg text-gray-800" x-text="material.training_title"></h2>
                                    </a>
                                @else
                                    <a :href="`{{ route('course.details', '') }}/${material.id}`">
                                ...
                                <p class="mt-1 text-yellow-500">
                                    <template x-for="i in 5">
                                        <span
                                            :class="i <= Math.floor(material.rating ?? 0) ? '' : 'text-gray-300'">★</span>
                                    </template>
                                    <span class="text-gray-700 font-medium ml-2">(<span
                                            x-text="material.rating ?? 0"></span>/5)</span>
                                </p>
                            </div>
                            ...
                            <div class="flex items-center text-sm text-gray-700 space-x-6 mt-4 flex-wrap">
                                ...
                                <!-- Level -->
                                <div>📈 <span x-text="material.training_level ?? '{{ langLabel('beginner') }}'"></span>
                                </div>

                                <!-- Session -->
                                <div>🎥 <span x-text="material.session_type ?? '{{ langLabel('recorded') }}'"></span>
                                </div>
                            </div>
                        </div>

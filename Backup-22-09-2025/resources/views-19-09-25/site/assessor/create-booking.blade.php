            ...
            <div class="flex-1 flex flex-col">
                ...
                <main class="p-6 bg-gray-100 flex-1 overflow-y-auto">
                    ...
                    <div class="bg-white rounded-lg shadow mt-6">
                        ...
                        <form id="slot-form" method="POST" action="{{ route('assessor.submit-bookings') }}" enctype="multipart/form-data" novalidate>
                            @csrf
                            <div class="p-6 space-y-4">
                                <!-- Select inputs -->
                                <div class="grid md:grid-cols-3 gap-4">
                                    <div>
                                        <label class="block mb-1 text-sm font-medium text-gray-700">{{ langLabel('coaching') }} {{ langLabel('mode') }}</label>
                                    ...
                                    <div>
                                        <label class="block mb-1 text-sm font-medium text-gray-700">{{ langLabel('start_timing') }}</label>
                                        <select id="startTime" class="w-full border border-gray-300 rounded-md px-3 py-2" required>
                                            <option value="">{{ langLabel('start_timing') }}</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label class="block mb-1 text-sm font-medium text-gray-700">{{ langLabel('end_timing') }}</label>
                                        <select id="endTime" class="w-full border border-gray-300 rounded-md px-3 py-2" required>
                                            <option value="">{{ langLabel('end_timing') }}</option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <script>
                            ...
                            function addSlot() {
                                ...
                                if (!mode) {
                                    showError("{{ langLabel('please_select_catching_mode') }}");
                                    return;
                                }

                                if (!start || !end) {
                                    showError("{{ langLabel('please_select_both_start_end_time') }}");
                                    return;
                                }

                                if (start === end) {
                                    showError("{{ langLabel('start_end_time_cannot_same') }}");
                                    return;
                                }
